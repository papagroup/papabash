...
Backup file           : $D_CONTAINER_NAME.$TODAY.tar.gz"

    FILE_TO_BACKUP="$LOCAL_BACKUP_DESTINATION/$D_NAME/$D_CONTAINER_NAME.$TODAY.tar.gz"
    ...
    FILE_TO_REMOVE="$LOCAL_BACKUP_DESTINATION/$D_NAME/$D_CONTAINER_NAME.$EXPIRED_DATE.tar.gz"
    ...
    [[ ! -e "/tmp/$D_NAME" ]] && mkdir -p "/tmp/$D_NAME"
...
function get_backup_file_from_drive() {
    ...
    GFILE_TO_RESTORE="$D_CONTAINER_NAME.$RESTORE_DATE.tar.gz"
...
function restore_docker_volume() {
    ...
    [ ! -f "$FILE_TO_RESTORE" ] && FILE_TO_RESTORE="/tmp/$D_NAME/$D_CONTAINER_NAME.$RESTORE_DATE.tar.gz"
    ...
    if [[ -z $(command -v slacktee.sh) ]]; then
        ...
        y | Y)
            ...
            if [[ "$result" == "0" ]]; then
                ...
                cp /usr/local/bin/slacktee.sh /usr/bin/slacktee.sh
                echo "slacktee.sh copied to /usr/bin/"
            else
...
function gdrive_delete_expired_backup() {
    echo " "
    echo "[Gdrive] Finding expired backup ($D_CONTAINER_NAME.$EXPIRED_DATE.tar.gz)..."
    GDRIVE_EXPIRED_BACKUP_ID=$(gdrive list --no-header --name-width 0 --query "name = '$D_CONTAINER_NAME.$EXPIRED_DATE.tar.gz'" --max 1 --order createdTime | awk '{ print $1 }')
    ...
    if [[ ! -z "$GDRIVE_EXPIRED_BACKUP_ID" ]]; then
        __log "[Gdrive] Found expired backup ($D_CONTAINER_NAME.$EXPIRED_DATE.tar.gz)."
...
function send_backup_to_gdrive() {
    ...
    FILE_TO_BACKUP="$LOCAL_BACKUP_DESTINATION/$D_NAME/$D_CONTAINER_NAME.$TODAY.tar.gz"
    if [ -f "$FILE_TO_BACKUP" ]; then
        __log "Backup file           : $D_CONTAINER_NAME.$TODAY.tar.gz ($(stat -c%s $FILE_TO_BACKUP | awk '{ split( "B KB MB GB TB PB" , v ); s=1; while( $1>1000 ){ $1/=1000; s++ } printf "%.2f %s", $1, v[s] }'))"
    else
        __log "Backup file           : $D_CONTAINER_NAME.$TODAY.tar.gz (Not found)"
    fi
    ...
    echo "[Gdrive] Checking if file already exists on gdrive - $D_CONTAINER_NAME.$TODAY.tar.gz"
    GDRIVE_FILE_ID=$(gdrive list --no-header --name-width 0 --query "name = '$D_CONTAINER_NAME.$TODAY.tar.gz'" --max 1 --order createdTime | awk '{ print $1 }')
    ...
    if [[ -z "$GDRIVE_FILE_ID" ]]; then
        ...
        echo "[Gdrive][Warn] $D_CONTAINER_NAME.$TODAY.tar.gz (id: $GDRIVE_FILE_ID) already exists."
        if [[ $YES_FOR_ALL = 1 ]]; then
            # All yes -> overdo
            GDRIVE_WRITE_ON=1
        else
    ...
    if [[ $GDRIVE_WRITE_ON = 1 ]]; then
        # Copy to temp folder
        cp -f $FILE_TO_BACKUP /tmp/$D_NAME/$D_CONTAINER_NAME.$TODAY.tar.gz
        # Upload temp file & delete if success
        __log "[Gdrive] Uploading the file to the folder..."

        GDRIVE_RESPONSE=$(gdrive upload --parent $GDRIVE_CONTAINER_DIR_ID --delete /tmp/$D_NAME/$D_CONTAINER_NAME.$TODAY.tar.gz)
        __log "[Gdrive][Debug] $GDRIVE_RESPONSE"

        # Temp file deleted := upload successfully
        if [[ ! -f /tmp/$D_NAME/$D_CONTAINER_NAME.$TODAY.tar.gz ]]; then
            __log "[Gdrive] $D_CONTAINER_NAME's Volume Backup Successfully Done."
            __log "[Gdrive] File Name: $D_CONTAINER_NAME.$TODAY.tar.gz ($(stat -c%s $FILE_TO_BACKUP | awk '{ split( "B KB MB GB TB PB" , v ); s=1; while( $1>1000 ){ $1/=1000; s++ } printf "%.2f %s", $1, v[s] }'))"
