...
create_droplet_res=$(curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" \
    -d '{"name":"${PROJECT_NAME}","region":"sgp1","size":"s-1vcpu-2gb","image":"docker-18-04","ssh_keys":[25333604,27610331,27610358],"backups":false,"ipv6":true,"user_data":null,"private_networking":null,"volumes": null,"tags":["web","python","django","saleor","channels","docker","dnkitchen"]}' \
...
if file --mime-type "$dbFile" | grep -q gzip$; then
  ...
  bash /root/papabash/docker-helpers/docker_backup.sh --destination "/home/.db_backups" --restore --container "securityalarm-webservices_db_1" --volume-dir "/var/lib/postgresql/data"
